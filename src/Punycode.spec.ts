...
describe('Punycode encoder/decoder', () => {
	describe('Given an encoder', () => {
		for (const { i, test: { decoded, normalized } } of justAscii.map((test, i) => ({ test, i }))) {
			describe(`And a text with just ascii [${i}]`, () => {
				it('Successfully encode strings', () => {
					expect(punycode.encode(decoded)).to.equal(normalized)
				})
			})
		}

		for (const [name, { decoded, normalized }] of Object.entries(examples)) {
		...
		for (const { i, test: { decoded, normalized } } of japaneseExamples.map((test, i) => ({ test, i }))) {
